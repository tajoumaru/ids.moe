...
import { addRateLimitHeaders, authenticateRequest } from "./auth";
import { handleGetApiKey, handleRegenerateApiKey } from "./handlers/apikey";
import { handleDataRoute } from "./handlers/data";
import { handlePlatformRoute, handleTMDBRoute, handleTraktRoute } from "./handlers/platform";
import { handleRedirect } from "./handlers/redirect";
import { handleHeartbeat, handleSchema, handleStatus, handleUpdated } from "./handlers/status";
import type { RouteContext } from "./types";
import { createCorsResponse, createErrorResponse, createTextResponse } from "./utils";
    ...
    try {
        ...
        switch (true) {
            ...
            case path === "/heartbeat" || path === "/ping":
                response = await handleHeartbeat(context);
                break;
            ...
            case path.startsWith("/data/"):
                response = await handleDataRoute(context, path);
                break;
