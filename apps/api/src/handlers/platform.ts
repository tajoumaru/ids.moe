...
export async function handlePlatformRoute(context: RouteContext, path: string): Promise<Response> {
    ...
    if (parts.length === 1) {
        ...
        if (path.endsWith(".tsv")) {
            return createTextResponse("TSV endpoint - implement KV-based TSV generation", 200);
        }
    ...
    if (parts.length === 2) {
        ...
        try {
            ...
            if (!animeData) {
                return createErrorResponse("Not found", 404, `Platform ${platform} with ID ${platformId} not found`);
            }

            return createJsonResponse(animeData);
        } catch (error) {
            console.error(`Error fetching anime data for ${platform}:${platformId}:`, error);
            return createErrorResponse("Not found", 404, `Platform ${platform} not found or not supported`);
        }
    }
...
export async function handleTraktRoute(context: RouteContext, path: string): Promise<Response> {
    ...
    if (parts.length < 3) {
        return createErrorResponse("Invalid request", 400, "Invalid Trakt URL format");
    }
    ...
    if (seasonId === "0" && (mediaType === "show" || mediaType === "shows")) {
        return createErrorResponse("Invalid season ID", 400, "Season ID cannot be 0");
    }
...
export async function handleTMDBRoute(context: RouteContext, path: string): Promise<Response> {
    ...
    if (parts.length < 3) {
        return createErrorResponse("Invalid request", 400, "Invalid TMDB URL format");
    }
    ...
    if (mediaType === "tv" || (parts.length > 3 && parts[3] === "season")) {
        return createErrorResponse("Invalid request", 400, "Currently, only `movie` are supported");
    }
    ...
    try {
        ...
        if (!animeData) {
            return createErrorResponse("Not found", 404, `Media type ${mediaType} with ID ${mediaId} not found`);
        }
