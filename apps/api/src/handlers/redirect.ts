...
export async function handleRedirect(context: RouteContext): Promise<Response> {
    ...
    if (resolvedTarget && !isValidTarget(resolvedTarget)) {
        return createErrorResponse("Invalid target", 400, `Target ${resolvedTarget} not found`);
    }
    ...
    try {
        ...
        return createErrorResponse("Not found", 404, `Platform ${resolvedPlatform} with ID ${platformId} not found`);
    }
}
