...
import type { HeartbeatResponse, RouteContext, StatusSchema } from "../types";
import { createErrorResponse, createJsonResponse, createTextResponse } from "../utils";
...
export async function handleHeartbeat(context: RouteContext): Promise<Response> {
    ...
    try {
        ...
        if (!testKey) {
            return createErrorResponse("Internal server error", 500, "KV data is corrupted or unavailable");
        }

        // Quick integrity check - verify we can get the actual data
        const testData = await context.env.IDS_KV.get(testKey);
        if (!testData) {
            return createErrorResponse("Internal server error", 500, "Data integrity check failed");
        }
        ...
        const responseTimeMs = healthCheckEnd - healthCheckStart;

        // Use the HeartbeatResponse type from the schema
        ...
        return createErrorResponse("Internal server error", 500, "Health check failed");
    }
}
...
export async function handleSchema(): Promise<Response> {
    ...
    const fallbackSchema = {
        ...
        info: {
            title: "ids.moe API",
            description:
                "High-performance anime ID mapping API providing cross-platform anime database ID relationships.",
            version: "2.0.0",
            license: {
                name: "AGPL-3.0-only",
...
export async function handleUpdated(context: RouteContext): Promise<Response> {
    try {
        ...
        if (lastUpdatedStr) {
            const timestamp = parseInt(lastUpdatedStr, 10);
