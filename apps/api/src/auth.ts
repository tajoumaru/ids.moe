...
import type { AuthUser, Env, RateLimitInfo } from "./types";
...
export async function verifyClerkToken(token: string, env: Env): Promise<ClerkSession | null> {
    ...
    try {
        ...
        await env.AUTH_CACHE.put(cacheKey, JSON.stringify(session), { expirationTtl: CACHE_TTL });
...
export async function checkRateLimit(
    ...
    const limit =
        customLimit || DEFAULT_RATE_LIMITS[tier as keyof typeof DEFAULT_RATE_LIMITS] || DEFAULT_RATE_LIMITS.free;
...
export async function authenticateRequest(
    ...
    if (isApiKeyEndpoint) {
        ...
        if (!session) {
            return {
                user: null,
                error: createErrorResponse("Unauthorized", 401, "Invalid session token"),
            };
        }
        ...
        if (!user) {
            ...
            if (!session) {
                return {
                    user: null,
                    error: createErrorResponse("Unauthorized", 401, "Invalid API key"),
                };
            }
    ...
    // Check rate limit
    const rateLimitInfo = await checkRateLimit(user.userId, user.tier, user.rateLimit, env);
